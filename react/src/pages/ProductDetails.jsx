        ...
        <div className="flex flex-col">
          {product.category_name && (
            <Link
              ...
              className="text-xs font-bold uppercase tracking-wider text-brand-600 hover:text-brand-700 dark:text-brand-400 dark:hover:text-brand-300"
            >
              {product.category_name}
            </Link>
          )}

          <h1 className="mt-2 text-2xl font-extrabold leading-tight tracking-tight text-slate-900 dark:text-slate-100 sm:text-3xl lg:text-4xl">
            {product.title}
          </h1>

          <div className="mt-5 flex flex-wrap items-center gap-4">
            <span className="text-3xl font-extrabold text-slate-900 dark:text-slate-100 sm:text-4xl">
            ...
            <div className="mt-8">
              ...
              <div className="inline-flex items-center rounded-lg border border-slate-300 bg-white dark:border-slate-700 dark:bg-slate-800">
                <button
                  ...
                  disabled={qty <= 1}
                  className="flex h-11 w-11 items-center justify-center text-slate-600 transition hover:bg-slate-100 disabled:opacity-40 dark:text-slate-300 dark:hover:bg-slate-700"
                >
                  <Minus size={16} />
                </button>
                <span className="w-14 text-center text-sm font-bold text-slate-900 dark:text-slate-100">{qty}</span>
                <button
          ...
          <div className="mt-7 flex flex-col gap-3">
            <div className="flex gap-3">
              <button onClick={handleAdd} disabled={outOfStock} className="btn-outline flex-1 !py-3 text-base">
                <ShoppingCart size={18} /> Add to cart
              </button>
              <button onClick={handleBuyNow} disabled={outOfStock} className="btn-primary flex-1 !py-3 text-base">
                Buy now
              </button>
            </div>
          </div>

          <div className="mt-8 grid grid-cols-1 gap-3 border-t border-slate-200 pt-6 dark:border-slate-800 sm:grid-cols-3">
            {[
              ...
              <div key={title} className="flex items-center gap-3">
                ...
                <div className="min-w-0">
                  <p className="truncate text-xs font-semibold text-slate-800 dark:text-slate-200">{title}</p>
                  <p className="truncate text-[11px] text-slate-500 dark:text-slate-400">{text}</p>
                </div>
              </div>
            ))}
          </div>
